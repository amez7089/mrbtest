...
print u"****Case--AmezMallH5_012_ReceivesCoupons买家领取优惠券--开始运行****"
...
try:
    ...
    time.sleep(1)
    #在新增优惠券页面输入名称：
    CouponsName =  u"满1000减999_" + str(datetime.now())
    print "CouponsName:",CouponsName
    ws.write(316,5,CouponsName)
    ...
    #输入面额
    driver.find_element_by_xpath("//*[@id='main']/div[2]/div[2]/form/div[4]/div/div/input").send_keys("999")
    time.sleep(1)
    #输入可发放总数
    ...
    #消费金额为2000
    driver.find_element_by_xpath("//*[@id='main']/div[2]/div[2]/form/div[7]/div/div/input").send_keys("1000")
    time.sleep(1)
    #代金券描述：
    driver.find_element_by_xpath("//*[@id='main']/div[2]/div[2]/form/div[8]/div/div/textarea").send_keys(u"满1000减999")
    time.sleep(1)
    #上传图片
...
#买家登录领取优惠券
    ...
    if (SearchGoods == u"小米（MI）小米电视4X 55英寸 L55M5-AD 2GB+8GB HDR 4K超高清 蓝牙语音遥"):
        ...
        for i in range(0,100):
                ...
                if (u"本店满1000减999" in CouponsList):
...
#买家进入我的--优惠券页面查看优惠券是否存在
    ...
    if (couponsName3==u"满1000可使用"):
        print u"优惠券领取成功，状态为待使用！"
        ws.write(333,5,couponsName3)
        print (u"Case--AmezMallH5_012_ReceivesCoupons买家领取优惠券---结果：Pass!")
        ws.write(310,7, 'Pass')
        #如果成功，将错误日志覆盖
        ws.write(310,10,'')
    else:
        print u"优惠券领取失败，不存在刚才领取的优惠券！"
        print (u"Case--AmezMallH5_012_ReceivesCoupons买家领取优惠券---结果：Failed!")
        ws.write(310,7, 'Failed',style2)
        ws.write(310,10, u"优惠券领取失败！！",style2)
    #关闭浏览器
    driver.quit()
...
#商家登录回滚数据，将优惠券置为无效
    ...
    ws.write(310,10,errorInfo,style2)
    
finally :
    if(errorFlag == 0):
        print (u"Case--AmezMallH5_012_ReceivesCoupons买家领取优惠券---结果：Failed!")
        ws.write(310,7, 'Failed',style2)
    ws.write(310,9, 'zhouchuqi')
    ws.write(310,8, datetime.now(), style1)    
    #利用保存时同名覆盖达到修改excel文件的目的,注意未被修改的内容保持不变
    wb.save('E:\\gitworksqace\\mrbdome1\\test1\\amez_test\\H5TestData.xls')
    print u"****Case--AmezMallH5_012_ReceivesCoupons.py--结束运行****"