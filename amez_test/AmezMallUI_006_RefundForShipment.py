...
print u"****Case--AmezMallUI_006_RefundForShipment 待发货订单退款，商家同意--开始运行****"
...
try:
    ...
    time.sleep(2)    
    #选择退款原因
    Select(driver.find_element_by_xpath("//*[@id='am_content']/div/div[2]/div[2]/div/div/div/div[4]/div[2]/div[2]/form/label[3]/select")).select_by_value("商品瑕疵")
    #填写退款说明
    driver.find_element_by_xpath("//*[@id='am_content']/div/div[2]/div[2]/div/div/div/div[4]/div[2]/div[2]/form/label[5]/textarea").send_keys(u"商品运输中损坏，申请退款！")
    #提交申请
    ...
    print u"---***商家用户登录***---"
    #读取用户名
    ShopuserName = table.cell(67,5).value
    print ShopuserName
    #读取密码
    ShoppassWord = table.cell(68,5).value
    print ShoppassWord
    #因为窗口变化，所以要定位当前的句柄，不然无法找到元素
    for handle in driver.window_handles:
        driver.switch_to_window(handle)
...
#     print type(orderNumber1)
    ...
    driver.find_element_by_xpath("//*[@id='main']/div[2]/ul/li[1]/div[2]/div[2]/div/textarea").send_keys(u"商品运输中损坏，同意退款！")
    #点击提交意见
    ...
    if (orderStatus == u"退款成功"):
        print u"退款成功！！！"
        print (u"Case--AmezMallUI_006_RefundForShipment 待发货订单退款，商家同意---结果：Pass!")
        ws.write(59,7, 'Pass')
        #如果成功，将错误日志覆盖
        ws.write(59,10,'')
    else:
        print u"退款失败！！！"
        print (u"Case--AmezMallUI_006_RefundForShipment 待发货订单退款，商家同意---结果：Failed!")
        ws.write(59,7, 'Failed',style2)
        ws.write(59,10,u'退款失败',style2)
    errorFlag = 1
    ...
    ws.write(59,10,errorInfo,style2)
    
finally :
    if(errorFlag == 0):
        print (u"Case--AmezMallUI_006_RefundForShipment 待发货订单退款，商家同意---结果：Failed!")
        ws.write(59,7, 'Failed',style2)
    ws.write(59,9, 'zhouchuqi')
    ws.write(59,8, datetime.now(), style1)    
    #利用保存时同名覆盖达到修改excel文件的目的,注意未被修改的内容保持不变
    wb.save('E:\\gitworksqace\\mrbdome1\\test1\\amze_test\\UITestData.xls')
    #退出浏览器
    driver.quit()
    print u"****Case--AmezMallUI_006_RefundForShipment 待发货订单退款，商家同意--结束运行****"