...
print u"****Case--AmezMallUI_007_RefundForGoodsToBeReceived 待收货订单退款，商家同意--开始运行****"
...
#     print type(orderNumber)
    print "orderNumber:",orderNumber
    #保存订单编号
    ws.write(76,5,orderNumber)
    
    #打开新窗口，商家登录发货
    NewWindow = 'window.open("http://company.test.amyun.cn");'
    driver.execute_script(NewWindow)
    #读取用户名
    ShopuserName = table.cell(78,5).value
    print ShopuserName
    #读取密码
    ShoppassWord = table.cell(79,5).value
    print ShoppassWord
    #因为窗口变化，所以要定位当前的句柄，不然无法找到元素
    for handle in driver.window_handles:
        driver.switch_to_window(handle)
    ...
    #填写物流单号
    trackingNumber = table.cell(83,5).value
    ...
    #退款原因
    Select(driver.find_element_by_xpath("//*[@id='am_content']/div/div[2]/div[2]/div/div/div/div[4]/div[2]/div[2]/form/label[3]/select")).select_by_value(u"收到商品时有划痕或破损")
    #退款说明
    driver.find_element_by_xpath("//*[@id='am_content']/div/div[2]/div[2]/div/div/div/div[4]/div[2]/div[2]/form/label[5]/textarea").send_keys(u"已发货未收货退款，仅退款，申请退款！")
    #点击提交
    ...
    driver.find_element_by_xpath("//*[@id='main']/div[2]/ul/li[1]/div[2]/div[2]/div/textarea").send_keys(u"情况属实，同意退款！")
    #提交
    ...
    if (orderStatus == u"退款成功"):
        print u"退款成功！！！"
        print (u"Case--AmezMallUI_007_RefundForGoodsToBeReceived 待收货订单退款，商家同意---结果：Pass!")
        ws.write(75,7, 'Pass')
        #如果成功，将错误日志覆盖
        ws.write(75,10,'')
    else:
        print u"退款失败！！！"
        print (u"Case--AmezMallUI_007_RefundForGoodsToBeReceived 待收货订单退款，商家同意---结果：Failed!")
        ws.write(75,7, 'Failed',style2)
        ws.write(75,10, u'退款失败')
    errorFlag = 1
...
#     traceback.print_exc()
    ...
    ws.write(75,10,errorInfo,style2)
    
finally :
    if(errorFlag == 0):
        print (u"Case--AmezMallUI_007_RefundForGoodsToBeReceived 待收货订单退款，商家同意---结果：Failed!")
        ws.write(75,7, 'Failed',style2)
    ws.write(75,9, 'zhouchuqi')
    ws.write(75,8, datetime.now(), style1)    
    #利用保存时同名覆盖达到修改excel文件的目的,注意未被修改的内容保持不变
    wb.save('E:\\gitworksqace\\mrbdome1\\test1\\amze_test\\UITestData.xls')
    #退出浏览器
    driver.quit()
    print u"****Case--AmezMallUI_007_RefundForGoodsToBeReceived 待收货订单退款，商家同意--结束运行****"