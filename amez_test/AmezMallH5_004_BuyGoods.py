...
print u"****Case--AmezMallH5_004_BuyGoods购买商品并付款流程--开始运行****"
...
try:
    ...
    if (SearchGoods == u"小米（MI）小米电视4X 55英寸 L55M5-AD 2GB+8GB HDR 4K超高清 蓝牙语音遥"):
        ...
        if (OrderGoods == u"小米（MI）小米电视4X 55英寸 L55M5-AD 2GB+8GB HDR 4K超高清 蓝牙语音遥" and OrderStatus == u"待发货"):
            print u"订单支付成功，CasePass！！！"
            print (u"Case--AmezMallH5_004_BuyGoods购买商品并付款流程--结果：Pass!")
            ws.write(35,7, 'Pass')    
            #如果成功，将错误日志覆盖
            ws.write(35,10,'')
        else:
            print (u"Case--AmezMallH5_004_BuyGoods购买商品并付款流程--结果：Failed!")
            ws.write(35,7, 'Failed',style2)   
    errorFlag = 1
    ...
    ws.write(35,10,errorInfo,style2)
    ...
    if(errorFlag == 0):
        ...
        ws.write(35,7, 'Failed',style2)
    #写入执行人员
    ws.write(35,9, 'zhouchuqi')
    #写入执行日期
    ws.write(35,8, datetime.now(), style1)    
    #利用保存时同名覆盖达到修改excel文件的目的,注意未被修改的内容保持不变
    wb.save('E:\\gitworksqace\\mrbdome1\\test1\\amze_test\\H5TestData.xls')
    #退出浏览器
    driver.quit()
    print u"Case--AmezMallH5_004_BuyGoods.py运行结束！！！"