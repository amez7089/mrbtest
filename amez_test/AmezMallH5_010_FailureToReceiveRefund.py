...
print u"****Case--AmezMallH5_010_FailureToReceiveRefund待收货订单退款（仅退款），商家拒绝--开始运行****"
...
#商家用户登录商家端查看订单并发货
    ...
    if (OrderNumber1 == OrderNumber):
        ...
        #填写物流单号
        trackingNumber = table.cell(248,5).value
...
#买家登录到待收货页面申请退款
    ...
    print "提取后的RefundNumber1:",OrderNumber3
    if (OrderNumber3 == OrderNumber):
        print u"待收货中存在需要的订单：",OrderNumber3
        #保存订单号
        ws.write(252,5,OrderNumber3)
        #申请退款
        ...
        driver.execute_script("arguments[0].click();", el)
        time.sleep(1)
        el=driver.find_element_by_xpath("//*[@id='radio' and @value='12']")
        driver.execute_script("arguments[0].click();", el)
        time.sleep(1)
        el=driver.find_element_by_xpath("//*[@id='moneyBack']/section/div[2]/div[2]")
        driver.execute_script("arguments[0].click();", el)
        time.sleep(1)
        #填写退款说明
        driver.find_element_by_xpath("//*[@id='moneyBack']/textarea").send_keys(u"不想等了，退款！")
        time.sleep(1)
        #确定
...
#商家用户登录处理退款请求
    ...
    time.sleep(2) 
    #获取订单编号 
    OrderNumber4 = driver.find_element_by_xpath("//*[@id='main']/div[2]/div[3]/div/div[3]/table/tbody/tr[1]/td[1]/div/div/ul/li[2]/p[2]/span[2]").text
    print u"商家退款记录页面第一条订单号为:",OrderNumber1
    if (OrderNumber4 == OrderNumber):
        print u"订单正确！！！"
        #保存订单号
        ws.write(260,5,OrderNumber4)
        #点击处理
...
#买家进入售后退款页面查看订单状态是否为退款失败：
    ...
    if (RefundNumber1 == RefundNumber and OrderStatus2 == u"商家已拒绝，退款失败"):
        print u"买家申请退款被拒绝！！！"
        #保存退款编号
        ws.write(268,5,RefundNumber1)
        #保存退款状态
        ws.write(267,5,OrderStatus2)
        print (u"Case--AmezMallH5_010_FailureToReceiveRefund待收货订单退款（仅退款），商家拒绝---结果：Pass!\n")
        ws.write(229,7, 'Pass')
        #如果成功，将错误日志覆盖
        ws.write(229,10,'')
    else:
        print u"买家申请退款未被拒绝！！！"
        print (u"Case--AmezMallH5_010_FailureToReceiveRefund待收货订单退款（仅退款），商家拒绝---结果：Fail!")
        ws.write(229,7, 'Failed',style2)  
        ws.write(229,10, u'买家申请退款失败',style2)
    #将失败标志置为1，表示脚本执行完成           
    errorFlag = 1
    ...
    ws.write(229,10,errorInfo,style2)
    
finally :
    if(errorFlag == 0):
        print u"Case--AmezMallH5_010_FailureToReceiveRefund待收货订单退款（仅退款），商家拒绝--结果：Failed!"
        ws.write(229,7, 'Failed',style2)
    #写入执行人员
    ws.write(229,9, 'zhouchuqi')
    #写入执行日期
    ws.write(229,8, datetime.now(), style1)    
    #利用保存时同名覆盖达到修改excel文件的目的,注意未被修改的内容保持不变
    wb.save('E:\\gitworksqace\\mrbdome1\\test1\\amez_test\\H5TestData.xls')
    #退出浏览器
    driver.quit()
    print u"Case--AmezMallH5_010_FailureToReceiveRefund.py运行结束！！！"