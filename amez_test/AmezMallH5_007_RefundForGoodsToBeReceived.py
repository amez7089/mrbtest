...
print u"****Case--AmezMallH5_007_RefundForGoodsToBeReceived待收货订单退款（仅退款），商家同意--开始运行****"
...
#商家用户登录商家端查看订单并发货
    ...
    if (OrderNumber1 == OrderNumber):
        ...
        #填写物流单号
        trackingNumber = table.cell(123,5).value
...
#买家进入售后退款页面查看订单状态是否为退款成功：
    ...
    if (RefundNumber1 == RefundNumber and OrderStatus2 == u"卖家已同意，退款成功"):
        print u"买家申请退款成功！！！"
        #保存退款编号
        ws.write(142,5,RefundNumber1)
        #保存退款状态
        ws.write(143,5,OrderStatus2)
        print (u"Case--AmezMallH5_007_RefundForGoodsToBeReceived待收货订单退款（仅退款），商家同意---结果：Pass!")
        ws.write(104,7, 'Pass')
        #如果成功，将错误日志覆盖
        ws.write(104,10,'')
    else:
        print u"买家申请退款失败！！！"
        print (u"Case--AmezMallH5_007_RefundForGoodsToBeReceived待收货订单退款（仅退款），商家同意---结果：Fail!")
        ws.write(104,7, 'Failed',style2)  
        ws.write(104,10, u'买家申请退款失败',style2)
    #将失败标志置为1，表示脚本执行完成           
    errorFlag = 1
    ...
    ws.write(104,10,errorInfo,style2)
    
finally :
    if(errorFlag == 0):
        print u"Case--AmezMallH5_007_RefundForGoodsToBeReceived待收货订单退款（仅退款），商家同意--结果：Failed!"
        ws.write(104,7, 'Failed',style2)
    #写入执行人员
    ws.write(104,9, 'zhouchuqi')
    #写入执行日期
    ws.write(104,8, datetime.now(), style1)    
    #利用保存时同名覆盖达到修改excel文件的目的,注意未被修改的内容保持不变
    wb.save('E:\\gitworksqace\\mrbdome1\\test1\\amze_test\\H5TestData.xls')
    #退出浏览器
    driver.quit()
    print u"Case--AmezMallH5_007_RefundForGoodsToBeReceived.py运行结束！！！"