...
print u"****Case--AmezMallUI_011_FailedReceiptRefund已收货订单退款退货（退货退款），商家拒绝--开始运行****"
...
try:
    ...
    #读取用户名
    ShopuserName = table.cell(161,5).value
    print ShopuserName
    #读取密码
    ShoppassWord = table.cell(162,5).value
    print ShoppassWord
    #因为窗口变化，所以要定位当前的句柄，不然无法找到元素
    for handle in driver.window_handles:
        driver.switch_to_window(handle)
    ...
    #填写物流单号
    trackingNumber = table.cell(166,5).value
    ...
    time.sleep(2)
    orderNumber3 = driver.find_element_by_xpath("//*[@id='am_content']/div/div[2]/div[2]/div/div[2]/div[1]/div[1]/ul/li/dl/dd[1]").text
    #提取订单号
    ...
    if (orderStatus == u"退款失败"):
        print (u"Case--AmezMallUI_011_FailedReceiptRefund已收货订单退款退货（退货退款），商家拒绝---结果：Pass!")
        ws.write(158,7, 'Pass')
        #如果成功，将错误日志覆盖
        ws.write(158,10,'')
    else:
        print (u"Case--AmezMallUI_011_FailedReceiptRefund已收货订单退款退货（退货退款），商家拒绝---结果：Failed!")
        ws.write(158,7, 'Failed',style2)
    errorFlag = 1
    ...
    ws.write(158,10,errorInfo,style2)
    
finally :
    if(errorFlag == 0):
        print (u"Case--AmezMallUI_011_FailedReceiptRefund已收货订单退款退货（退货退款），商家拒绝---结果：Failed!")
        ws.write(158,7, 'Failed',style2)
    ws.write(158,9, 'zhouchuqi')
    ws.write(158,8, datetime.now(), style1)    
    #利用保存时同名覆盖达到修改excel文件的目的,注意未被修改的内容保持不变
    wb.save('E:\\gitworksqace\\mrbdome1\\test1\\amez_test\\UITestData.xls')
    #退出浏览器
    driver.quit()
    print u"****Case--AmezMallUI_011_FailedReceiptRefund已收货订单退款退货（退货退款），商家拒绝--结束运行****"